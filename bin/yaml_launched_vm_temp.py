...
def cleanup_vm(vm_name):
    ...
    image_path_debian = os.path.join("/data/VMs/debian", "debian_vm_test.qcow2")
    image_path_centos = os.path.join("/data/VMs/centos", "centos9_vm_test.qcow2")
    if os.path.exists(image_path_debian):
        os.system(f"sudo rm {image_path_debian}")
        logging.info(f"Cleaned up Debian VM image: {image_path_debian}")
    elif os.path.exists(image_path_centos):
        os.system(f"sudo rm {image_path_centos}")
        logging.info(f"Cleaned up Centos VM image: {image_path_centos}")
    else:
